...
def read_data(directory):
    ...
    with open(directory) as file:
        for ctx in file.read().split("\n"):
            data.append(re.sub('[\t\"]', ' ', ctx).strip())
    return data[:-1]
...
train_source = read_data('./train/source.csv')
train_target = read_data('./train/target.csv')
dev_source = read_data('./dev/source.csv')
dev_target = read_data('./dev/target.csv')
